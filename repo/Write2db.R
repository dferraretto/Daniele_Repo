...
field.data = function(file){
   ...
   through$depth = (through$vol * 10^6) / (4.02 * 0.114 * 2 * gutter + colander) / 10^6 # width corrected from .234 to .114, 24/03/16
   # Prep for db
...
lab.data = function(file){
   ...
   # get rid of the lab values before melting
   Nite$NO3.concentration.mg.L = NULL
   ...
   # get rid of the lab values before melting
   Nite$NO4.concentration.mg.L = NULL
