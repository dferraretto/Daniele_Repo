...
tf  = dbGetQuery(db, "SELECT date, sample, site, variable, vals FROM fielddata WHERE variable = 'through vol' AND vals>=0 ORDER BY date")
...
tf$mY=strftime(tf$date,"%Y%m")
tf$Y=strftime(tf$date,"%Y")
# OVERALL MEAN TF VOL
